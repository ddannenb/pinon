...
class Comps:
    def __init__(self, config):
        ...
        self.all_ks = None
        self.peer_ks = None
        # m = self.config.companies.index.isin(config.companies.loc['AXP'][pn_cols.PEER_LIST])
        # m = self.config.companies.loc[self.companies.index in self.config.companies.loc['AXP'][pn_cols.PEER_LIST]]
        # self.sum_peer_weights = self.config.companies[pn_cols.PEER_WEIGHT].sum()
